public class Tree {
    ...
    private void error(String errMess) throws BSTException{
		throw new BSTException(errMess);
	}
	...
	public Integer postorderEval(TreeNode node, String indent, BST symTab) throws BSTException{
    	...
    	if(node.getItem().matches("[0-9]+") && (node.getLeft() == null && node.getRight() == null) )//root is a number
    		...
    		try{//try used as symbol may not exist in symTab
    			res = symTab.retrieveItem(node.getItem()).getVal();}
    		catch(Exception e){
    			error("undefined identifier: " + node.getItem());
    		}}
