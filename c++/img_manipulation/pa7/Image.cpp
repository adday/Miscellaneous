...
int detectColorEdges(const Image & img, Image & E){
  ...
  for(int y=1;y<img.getHeight()-1;y++){//for each column
	...
	for(int x=1;x<img.getWidth()-1;x++){//for each row
		...
		ER = round(static_cast<double>(sqrt((IhR*IhR)+(IvR*IvR))) );
		min = ER<min ? ER: min;
		max = ER>max ? ER: max;
		...
		EG = round(static_cast<double>(sqrt((IhG*IhG)+(IvG*IvG))) );
		min = EG<min ? EG: min;
		max = EG>max ? EG: max;
		...
		EB = round(static_cast<double>(sqrt((IhB*IhB)+(IvB*IvB))) );
		min = EB<min ? EB: min;
		max = EB>max ? EB: max;
...
int detectEdges(const Image & img, Image & E){
  ...
  for(int y=0;y<img.getHeight();y++){//for each column
	for(int x=0;x<img.getWidth();x++){//for each row
		...
		E.setPixel(x,y, round(static_cast<double>(sqrt((Ih.getPixel(x,y)*Ih.getPixel(x,y)))
...
int Image::rescale(void){
 ...
 for(int i=0;i<height;i++)
	for(int j=0;j<width;j++)
	     pixels.at(i).at(j)=round(static_cast<double>(255*(pixels.at(i).at(j)-min)/(max-min))); 
 min = 0;
 max = 255;
 return 0;
}//end rescale
...
int Image::colorRescale(void){
 ...
 for(int i=0;i<height;i++){
	for(int j=0;j<width;j++){
	    colors.at(i).at(j).setRed(round(255*static_cast<double>(colors.at(i).at(j).getRed()-min)/(max-min))); 
	    colors.at(i).at(j).setGreen(round(255*static_cast<double>(colors.at(i).at(j).getGreen()-min)/(max-min))); 
	    colors.at(i).at(j).setBlue(round(255*static_cast<double>(colors.at(i).at(j).getBlue()-min)/(max-min)));
	 }//end for width
  }//end for height
 min = 0;
 max = 255;
...
string Image::binaryPPMParse(ifstream& istr){
  ...
  for(int i = 0; i < height; i++){
	...
	for(int j = 0; j < width; j++){
	    ...
	    if(blue < 0)
		return "Pixel found is less than 0";
            pixel = round(static_cast<double>((red+green+blue)/3));
